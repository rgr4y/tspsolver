/*
* tspsolver
* https://github.com/deftx/tspsolver
*
* Adapted from http://code.google.com/p/google-maps-tsp-solver/
*
* Copyright (c) 2013 Rob Vella
...
//        console.log("Waypoints: "+JSON.stringify(wayArrChunk2));
...
//                console.log("Error: " + err);
                ...
                if (data.status == "OK") {
                    ...
                    for (var i = 0; i < data.routes[0].legs.length; ++i) {
                        ...
                        legsTmp.push(data.routes[0].legs[i]);
                        durations.push(data.routes[0].legs[i].duration.value);
                        distances.push(data.routes[0].legs[i].distance.value);
                    }
                    if (typeof onProgressCallback == 'function') {
                        onProgressCallback();
                    }
                    ...
                    var errorMsg = data.status; // DIR_STATUS_MSG[directionsStatus];
...
function addAddress(address, label, callback) {
    ...
    gm.geocode(address, function(err, data) {
        if (data.status == "OK") {
            ...
            --addressRequests;
            console.log("Failed to geocode address: " + address + ". Reason: " + data.status);// + GEO_STATUS_MSG[status]);
            ++currQueueNum;
            addressProcessing = false;
            if (typeof(callback) == 'function')
                callback(address);
...
function createGoogleLink(gdir, embed) {
    var addr = exports.getAddresses();
    var order = exports.getOrder();
...
//function BpTspSolver(onFatalError) {
//    if (tsp) {
...
exports.addAddressWithLabel = function(address, label, callback) {
    ++addressRequests;
    ++requestNum;
    exports.addAddressAgain(address, label, callback, requestNum - 1);
}

exports.addAddress = function(address, callback) {
    exports.addAddressWithLabel(address, null, callback);
};

exports.addAddressAgain = function(address, label, callback, queueNum) {
    if (addressProcessing || queueNum > currQueueNum) {
        setTimeout(function(){ exports.addAddressAgain(address, label, callback, queueNum) }, 100);
        return;
    }
    addAddress(address, label, callback);
};

exports.addWaypointWithLabel = function(latLng, label, callback) {
    ++requestNum;
    exports.addWaypointAgain(latLng, label, callback, requestNum - 1);
};

exports.addWaypoint = function(latLng, callback) {
    exports.addWaypointWithLabel(latLng, null, callback);
};

exports.addWaypointAgain = function(latLng, label, callback, queueNum) {
    if (addressProcessing || queueNum > currQueueNum) {
        setTimeout(function(){ exports.addWaypointAgain(latLng, label, callback, queueNum) }, 100);
        return;
    }
...
exports.removeWaypoint = function(latLng) {
    for (var i = 0; i < waypoints.length; ++i) {
...
exports.removeAddress = function(addr) {
    for (var i = 0; i < addresses.length; ++i) {
...
exports.setAsStop = function(latLng) {
    var j = -1;
...
exports.setAsStart = function(latLng) {
    var j = -1;
    for (var i = 0; i < waypoints.length; ++i) {
        if (j == -1 && wpActive[i]) {
            j = i;
...
exports.getGDirections = function() {
    return gebDirectionsResult;
};

exports.getGDirectionsService = function() {
...
exports.getAvoidHighways = function() {
    return avoidHighways;
}

exports.setAvoidHighways = function(avoid) {
    avoidHighways = avoid;
}

exports.getAvoidTolls = function() {
    return avoidTolls;
}

exports.setAvoidTolls = function(avoid) {
    avoidTolls = avoid;
}

exports.getTravelMode = function() {
    return travelMode;
}

exports.setTravelMode = function(travelM) {
    travelMode = travelM;
}
...
// Helper functions
exports.getTotalDuration = function() {
...
exports.solveRoundTrip = function(callback) {
    if (doNotContinue) {
    ...
    if (!this.isReady()) {
        setTimeout(function(){ exports.solveRoundTrip(callback) }, 20);
        return;
    }
    if (typeof callback == 'function')
...
exports.solveAtoZ = function(callback) {
    if (doNotContinue) {
        console.log('Cannot continue after fatal errors.');
        return;
    }
    ...
    if (!this.isReady()) {
        setTimeout(function(){ exports.solveAtoZ(callback) }, 20);
        return;
    }
...
exports.setDirectionUnits = function(mOrKm) {
    if (mOrKm == "m") {
        directionunits = "imperial";
    }
    else {
        directionunits = "metric";
    }
}

exports.setOnProgressCallback = function(callback) {
    onProgressCallback = callback;
}

exports.getNumDirectionsComputed = function () {
    return numDirectionsComputed;
}

exports.getNumDirectionsNeeded = function () {
    return numDirectionsNeeded;
}

exports.reverseSolution = function () {
    reverseSolution();
}

exports.reorderSolution = function(newOrder, callback) {
    if (typeof callback == 'function')
        onSolveCallback = callback;
...
exports.removeStop = function(number, callback) {
    if (typeof callback == 'function')
        onSolveCallback = callback;
...
exports.createGoogleLink = function(embed) {
    var embed = embed ? true : false;
    return createGoogleLink(exports.getGDirections().routes[0], embed);
}